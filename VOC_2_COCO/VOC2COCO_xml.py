...
def convert(root_path, target_xml_root_path):
    """
    root_path:
    ...
    for mode in ['train', 'val', 'test']:
        ...
        if not os.path.exists(folder):
            os.makedirs(folder)
        json_name = os.path.join(target_xml_root_path, 'annotations/instances_{}sku.json'.format(mode))
        with open(json_name, 'w') as f_json:
            json.dump(dataset, f_json)


if __name__ == '__main__':
    convert(root_path='/home/lichengzhi/faster-rcnn.pytorch/data/VOCdevkit/SKU110K',
            target_xml_root_path='/home/lichengzhi/maskrcnn-benchmark/datasets/coco/SKU110K')
