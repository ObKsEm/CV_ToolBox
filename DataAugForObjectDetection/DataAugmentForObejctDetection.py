...
from DataAugForObjectDetection.xml_helper import parse_xml, generate_xml
    ...
    def _cutout(self, img, bboxes, length=100, n_holes=1, threshold=0.5):
        ...
        for n in range(n_holes):
            ...
            while chongdie:
                ...
                # numpy.clip(a, a_min, a_max, out=None), clip这个函数将将数组中的元素限制在a_min,
    ...
    def dataAugment(self, img, bboxes, polys):
        ...
        while change_num < 1:   # 默认至少有一种数据增强生效
            ...
            if random.random() < self.shift_rate:           # 平移
                print('平移')
                change_num += 1
                img, bboxes, polys = self._shift_pic_bboxes(img, bboxes)
            ...
            if random.random() < self.cutout_rate:          # cutout
                print('cutout')
                change_num += 1
                img = self._cutout(img, bboxes, length=self.cut_out_length,
                                   n_holes=self.cut_out_holes, threshold=self.cut_out_threshold)
